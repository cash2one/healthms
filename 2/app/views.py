...
from app import app
from models import *
from flask import g, request, render_template, abort, url_for, jsonify, session, redirect
...
def index():
    ...
    c.execute('SELECT b.* FROM healthms_ask_answer_doctors AS a RIGHT JOIN healthms_doctors_info AS b ON a.doctor_id = b.doctor_id WHERE b.doctor_check = 2 ORDER BY a.answer_doctors_true DESC LIMIT 6')
...
def list_doctors():
    c = g.db.cursor()
    c.execute('SELECT b.* FROM healthms_ask_answer_doctors AS a RIGHT JOIN healthms_doctors_info AS b ON a.doctor_id = b.doctor_id WHERE b.doctor_check = 2 ORDER BY a.answer_doctors_true DESC')
    doctors = list(c.fetchall())
    if 'patientEmail' in session:
        patientInfo = get_patient_info_session()
...
def ask_form_deal(userTyp):
    ...
    if userTyp == USER_TYP[0]:
        ...
        if 'doctorEmail' in session:
            ...
            askId = request.form.get('askId')
            commition = request.form.get('commition')
            c.execute('SELECT ask_doctor_id FROM healthms_ask_answer_doctors')
...
def ask_form_finish():
    if 'patientEmail' in session:
        ...
        c.execute('UPDATE healthms_ask_answer_doctors SET answer_doctors_true = answer_doctors_true + 1 WHERE ask_doctor_id = %s', [askId])
...
def admin_login():
    if request.method == 'POST':
        ...
        try:
            admin = healthms_admins_info.query.filter_by(admin_username = adminName).first()
            if adminName == admin.admin_username and adminPassword == base64.decodestring(admin.admin_password):
    ...
    if 'adminUsername' in session:
        if infoTyp in WEB_INFOS['INFO_TYP']:
            ...
            if request.method == 'GET':
                ...
                try:
                    article = healthms_info(info_type = infoTyp, info_title = infoTitle, info_content = infoContent, admin_id = adminInfo['adminId'])
...
def admin_info_del():
    if 'adminUsername' in session:
        try:
            ...
            admin = healthms_info.query.get(infoId)
...
def admin_info_list(infoTyp):
    if 'adminUsername' in session:
        if infoTyp in WEB_INFOS['INFO_TYP']:
            adminInfo = get_admin_info_session()
            articles = healthms_info.query.filter_by(info_type = infoTyp).all()
            return render_template('admin_info_list.html', WEB_INFOS = WEB_INFOS, adminInfo = adminInfo, articles = articles, infoTyp = infoTyp)
        else:
            abort(404)
    return redirect(url_for('admin_login'))

@app.route('/admin/info/deatil/<int:infoId>', methods=['GET', 'POST'])
def admin_info_detail(infoId):
    if 'adminUsername' in session:
        adminInfo = get_admin_info_session()
        if request.method == 'GET':
            article = healthms_info.query.get(infoId)
            if not article is None:
                isAdmin = False
                if article.admin.admin_id == adminInfo['adminId']:
                    isAdmin = True
                return render_template('admin_info_detail.html', WEB_INFOS = WEB_INFOS, adminInfo = adminInfo, article = article, isAdmin = isAdmin)
            else:
                abort(404)
        else:
            infoTitle = request.form.get('infoTitle')
            ...
            article = healthms_info.query.get(infoId)
            if not article is None:
                article.info_title = infoTitle
                article.info_content = infoContent
                article.info_type = infoTyp
                db.session.commit()
            return redirect(url_for('admin_info_list', infoTyp = infoTyp))
    return redirect(url_for('admin_login'))
...
def set_admin_info_session(admin):
    ''' 设置管理员 session 值，共5个 '''
    session['adminId'] = admin.admin_id
    session['adminUsername'] = admin.admin_username
    session['adminRank'] = admin.admin_rank
    session['adminLoginDate'] = admin.admin_login_date
    session['adminCreateDate'] = admin.admin_create_date
